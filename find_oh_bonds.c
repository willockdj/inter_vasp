...
//
        p_atom2=p_molecule;
        for (iatom2=0; iatom2< num_atoms; iatom2++)
              {
//                printf("%s %10.6f %10.6f %10.6f \n", p_atom->label);
//
//                //                printf("This atom has %d neighbours....\n", p_atom->num_neigh);
//
                 if ( strcmp(p_atom2->elem, "H") == 0)
                   {

                      dist = atom_separation_squared(p_atom1, p_atom2, use_pbc, p_recip_latt_vec, p_latt_vec);
                      dist = sqrt(dist);
 
                      printf("This is an Hydrogen atom %s (elem=%s) %10.6f from this O\n", p_atom2->label, p_atom2->elem, dist);
