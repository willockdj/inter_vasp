...
int read_input( FILE *input_fp, char *p_title, char *p_master_input,
                ...
                char *p_axis2_lab,
                double *p_mag_after_switch, int *p_super, int *p_need_shift,
                char_list *p_image_files, int *p_num_images,
...
int read_doscar( FILE *fp, dos *p_ndos, int need_pdos, int *p_part_dos_list, 
                 int num_atoms_pdos, dos *p_pdos, int *p_spd, int num_pdos_orb );
  ...
  char  **argv;
{
  ...
  e_vec eigenvecs[MAXMODES], forces, tan_forces;
  ...
  int iii,iloop, jloop, iatom, iatom2, icomp, icoord, iletter;
  int idos;
...
int need_angle, need_dos, num_dos_points, spd[3];
...
int num_inter, have_grp, have_out, check, check1, check2;
    ...
    if (have_grp) 
      {
        if (need_angle)
          {
             printf("Will rotate the GRUP atoms around the axis defined by the line\n");
...
/* Sum part_dos similarly */
     ...
     else
       {
          ...
          num_pdos_orb= count_doscar( fp_doscar_input, &temp_ndos[0], need_partdos, 
                                      &part_dos_list[0], num_atoms_pdos, &temp_pdos[0],
                                      &spd[0] );
     ...
     write_doscsv( fp_dos_output, title_x, title_y, 
                  title_z, &ndos[0], have_tot, num_dos_points-1);
...
/**** possibility of a range of atoms being present       ****/
         ...
         write_doscsv( fp_dos_output, title_x, title_y, 
                      title_z, &pdos[0], have_tot, num_dos_points-1);
...
/**** For master input check for fixing flags in group labels ****/
    ...
    good_read=  read_car( fp_car_input, &header_line[0], &title_line[0],
                          ...
                          &num_in_group, have_grp, TRUE, &fix_flags[0]);
...
/****************************************************************************/
    else if (have_out)
      {
        ...
        read_outcar( fp_outcar, &molecule[0], &latt_vec[0], 
                     ...
                     &eigenvecs[0], &forces, &tan_forces,
                     &num_atoms, &num_types, &num_modes, need_freq,
                     need_force, &have_band);
          ...
          if (have_band)
             {
                ...
                for (iatom=0; iatom < num_atoms; iatom++)
                  {
                     ...
                     rms_x += tan_forces.dx[iatom] * tan_forces.dx[iatom];
                     rms_y += tan_forces.dy[iatom] * tan_forces.dy[iatom];
                     rms_z += tan_forces.dz[iatom] * tan_forces.dz[iatom];
                  }
                rms_x = sqrt( rms_x / num_atoms);
                rms_y = sqrt( rms_y / num_atoms);
                rms_z = sqrt( rms_z / num_atoms);
                ...
                for (iatom=0; iatom < num_atoms; iatom++)
                  {
                     forces.dx[iatom]=forces.dx[iatom]-tan_forces.dx[iatom];
                     forces.dx[iatom]=forces.dy[iatom]-tan_forces.dy[iatom];
                     forces.dx[iatom]=forces.dz[iatom]-tan_forces.dz[iatom];
